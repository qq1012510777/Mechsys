...
public:
    ...
    bool FOUND_NEW_PNT_SPLIT_TRACE = false;
                                                                ...
                                                                std::vector<DFN::Line_seg_2D> Traces_Line)
{
    ...
    //cout << "dt.insert(vInputPoints);\nstd::vector<Segment2> vSegments(" << Topo_struct.size() << ");\n";
    ...
    std::vector<ConstraintSegment2 *> vConstraintSegments_;
    pCG->getChildConstraintSegments(vConstraintSegments_);

    Zone2 *pZoneGlobal(dt.createZone(NULL, ZL_GLOBAL));
    Zone2 *pBoundedZone(pZoneGlobal->convertToBoundedZone());

    MeshGenParams params(pBoundedZone);
    params.bAllowConstraintSplitting = true;
    params.maxEdgeLength = max_edge_tri;
    params.minEdgeLength = min_edge_tri;
    params.minAngleDegree = min_angle;

    //cout << "params.maxEdgeLength: " << params.maxEdgeLength << endl;
    //cout << "params.minEdgeLength: " << params.minEdgeLength << endl;
    //cout << "params.minAngleDegree: " << params.minAngleDegree << endl;

    /*
    for (size_t i = 0; i < Topo_struct_attri.size(); ++i)
    {
        if (Topo_struct_attri[i] == true)
        {
            params.addLockedConstraint(vConstraintSegments_[i]);
            //cout << "params.addLockedConstraint(vConstraintSegments_[" << i << "]);\n";
        }
    }*/
    ...
    for (size_t i = 0; i < vTriangles.size(); ++i)
    {
        ...
        if (PointID[0] == PointID[1] || PointID[0] == PointID[2] || PointID[1] == PointID[2])
        {
            ...
            Point2 *A0 = vTriangles[i]->getCorner(0);
            Point2 *A1 = vTriangles[i]->getCorner(1);
            Point2 *A2 = vTriangles[i]->getCorner(2);
    ...
    */
    ...
    {
        FOUND_NEW_PNT_SPLIT_TRACE = true;
        return;
    }
    else
        FOUND_NEW_PNT_SPLIT_TRACE = false;
...
inline bool Mesh_Polygon_2D_with_traces::If_new_pnt_generates_and_splits_traces(std::vector<DFN::Line_seg_2D> Traces_Line, std::vector<Vector2d> Trace_pnts)
{
    ...
    {
        DFN::Point_2D thisPNT{JXY[i]};
        for (size_t j = 0; j < Traces_Line.size(); ++j)
        {
            bool gh = thisPNT.If_lies_on_a_line_seg(Traces_Line[j].Point);
            ...
            if (gh == true)
            {
                for (size_t k = 0; k < Trace_pnts.size(); ++k)
                {
                    ...
                    if (abs(UIO(0)) < 0.01 && abs(UIO(1)) < 0.01)
                    {
                        break; //
                    }
                    else
