...
// MechSys
#include <mechsys/dfn_c/Fracture.h>
#include <mechsys/util/tree.h>
//#include <mechsys/dfn/Graph_i.h>
#include <mechsys/dfn_c/Graph_x.h>
...
namespace DFN
{
	...
	public:
		...
		Array<Array<size_t>> Percolation_cluster;									  ///< three orders, N dimensions; first order means x direction, second means y, third means z; alone a percolation direction, there might be zero or more clusters; each element is the subscript of array (Listofclusters)
		...
		void Create_whole_model(const size_t n, const Array<double> DenWeight, const gsl_rng *random_seed, const double model_size[6], const String str_ori, const String str_frac_size, const Array<Vec2_t> array11, Array<Vec4_t> array12, const Array<Vec7_t> array13, double P_end_x, double index_ak);
		...
		void Determine_excluded_volume(const String str_ori, const String str_frac_size, double x0, double x1, double alpha_g = 0, double kappa = 0, double mean = 0, double var = 0, double min_R = 0, double max_R = 0); ///<  <A*P>
	...
	inline void Domain::Create_whole_model(const size_t n, const Array<double> DenWeight, const gsl_rng *random_seed, const double model_size[6], const String str_ori, const String str_frac_size, const Array<Vec2_t> array11, const Array<Vec4_t> array12, const Array<Vec7_t> array13, double P_end_x, double index_ak)
	{
		P_end = P_end_x;
					...
					if (P_end < 2000)
					{
						...
						else
						{
							if (str_ori == "uniform")
							{
								if (str_frac_size == "powerlaw")
								{
									Determine_excluded_volume(str_ori, str_frac_size, array12[0][1], array12[0][2], array12[0][0], 0, 0, 0, 0, 0);
								}
								else if (str_frac_size == "lognormal")
								{
									Determine_excluded_volume(str_ori, str_frac_size, 0, 0, 0, 0, 0, 0, array12[0][0], array12[0][1]);
								}
								else if (str_frac_size == "uniform")
								{
									Determine_excluded_volume(str_ori, str_frac_size, 0, 0, 0, 0, 0, 0, array12[0][0], array12[0][1]);
								...
								{
									Determine_excluded_volume(str_ori, str_frac_size, 0, 0, 0, 0, 0, 0, array12[0][0], array12[0][0]);
								}
								else
								{
							...
							else if (str_ori == "fisher")
							{
								if (str_frac_size == "powerlaw")
								{
									Determine_excluded_volume(str_ori, str_frac_size, array12[0][1], array12[0][2], array12[0][0], array13[0][2], 0, 0, 0, 0);
								}
								else if (str_frac_size == "lognormal")
								{
									Determine_excluded_volume(str_ori, str_frac_size, array12[0][2], array12[0][3], 0, array13[0][2], array12[0][0], array12[0][1], 0, 0);
								}
								else if (str_frac_size == "uniform")
								{
									Determine_excluded_volume(str_ori, str_frac_size, 0, 0, 0, array13[0][2], 0, 0, array12[0][0], array12[0][1]);
								}
								else if (str_frac_size == "single")
								{
									Determine_excluded_volume(str_ori, str_frac_size, 0, 0, 0, array13[0][2], 0, 0, array12[0][0], array12[0][0]);
								}
								else
								{
									std::cout << "Error! You do not define fracture size distribution!\n";
						...
						if (abs(Percolation_parameter_a - P_end_x) <= 0.01)
						{
							/*
							std::cout << "The model's percolation parameter is: " << Percolation_parameter_a << std::endl;
							std::cout << "The model's P30 is: " << P30 << std::endl;
							*/
							break;
						}
						else if (Percolation_parameter_a > P_end_x + 0.01)
						{
							/*
							std::cout << "The model's percolation parameter is: " << Percolation_parameter_a << std::endl;
							std::cout << "The model's P30 is: " << P30 << std::endl;
							std::cout << "Fail to generate DFN model with your defined Percolation parameter value! automatically run again!\n";
							//Fractures.Clear();
							//i = 0;
							*/
							Percolation_parameter_a = -100;
							goto h100;
						}
					}
				}
...
#pragma omp critical
		...
		//std::cout << str_frac_size << std::endl;
		if (str_ori == "uniform")
		{
			if (str_frac_size == "powerlaw")
			{
	...
	inline void Domain::Determine_excluded_volume(const String str_ori, const String str_frac_size, double x0, double x1, double alpha_g = 0, double kappa = 0, double mean = 0, double var = 0, double min_R = 0, double max_R = 0)
	{
		if (str_ori == "uniform")
		{
			...
			if (str_frac_size == "powerlaw")
			{
				...
				//********below is 0.5*<A*P>
				Excluded_volume_1 = 8 * pow(2, 0.5) * C2 * pow(x1, 4 - alpha_g) / (4 - alpha_g) - 8 * pow(2, 0.5) * C2 * pow(x0, 4 - alpha_g) / (4 - alpha_g);
				Excluded_volume_1 = 0.5 * Excluded_volume_1;

				//********below is 0.5*<A>*<P>
				Excluded_volume_2 = (8 * pow(2, 0.5) * C2 * C2 * (pow(x1, 3 - alpha_g) / (3 - alpha_g) - pow(x0, 3 - alpha_g) / (3 - alpha_g)) * (pow(x1, 2 - alpha_g) / (2 - alpha_g) - pow(x0, 2 - alpha_g) / (2 - alpha_g))) * 0.5;

				//********below is 0.5 * <l^3>
				Excluded_volume_3 = 0.5 * 2 * pow(2, 0.5) * C2 * pow(x1, 4 - alpha_g) / (4 - alpha_g) - 2 * pow(2, 0.5) * C2 * pow(x0, 4 - alpha_g) / (4 - alpha_g);

				//********below is 0.5 * <l^3>/<l^2>, and note that the percolation parameter equals to P32*<l^3>/<l^2>
				Excluded_volume_4 = Excluded_volume_3 / (2 * C2 / (3 - alpha_g) * (pow(x1, 3 - alpha_g) - pow(x0, 3 - alpha_g)));

				//********below is 0.5 * <A*P>/<l^2>, and note that the percolation parameter equals to P32*<l^3>/<l^2>
				Excluded_volume_5 = Excluded_volume_1 / (2 * C2 / (3 - alpha_g) * (pow(x1, 3 - alpha_g) - pow(x0, 3 - alpha_g)));
			}
			else if (str_frac_size == "lognormal")
			{
				mean = log(mean);
				var = log(var);
				//********below is 0.5*<A*P>
				Excluded_volume_1 = 0.5 * 8 * pow(2, 0.5) * exp(3 * mean + (9 / 2) * (var * var));

				//********below is 0.5*<A>*<P>
				Excluded_volume_2 = 0.5 * 2 * exp(2 * mean + 2 * var * var) * 4 * pow(2, 0.5) * exp(mean + 0.5 * var * var);

				//********below is 0.5 * <l^3>
				Excluded_volume_3 = 0.5 * 2 * pow(2, 0.5) * exp(3 * mean + (9 / 2) * (var * var));

				//********below is 0.5 * <l^3>/<l^2>, and note that the percolation parameter equals to P32*<l^3>/<l^2>
				Excluded_volume_4 = Excluded_volume_3 / (2 * exp(2 * mean + 2 * var * var));

				//********below is 0.5 * <A*P>/<l^2>, and note that the percolation parameter equals to P32*<l^3>/<l^2>
				Excluded_volume_5 = Excluded_volume_1 / (2 * exp(2 * mean + 2 * var * var));
			}
			else if (str_frac_size == "uniform")
			{
				double C2 = 1 / (max_R - min_R);
				//********below is 0.5*<A*P>
				Excluded_volume_1 = 0.5 * 2 * pow(2, 0.5) * C2 * (pow(max_R, 4) - pow(min_R, 4));

				//********below is 0.5*<A>*<P>
				Excluded_volume_2 = 0.5 * C2 * C2 * (8 * pow(2, 0.5) / (2 * 3)) * (pow(max_R, 3) - pow(min_R, 3)) * (pow(max_R, 2) - pow(min_R, 2));

				//********below is 0.5 * <l^3>
				Excluded_volume_3 = 0.5 * 2 * pow(2, 0.5) * C2 * ((pow(max_R, 4) - pow(min_R, 4)) / 4);

				//********below is 0.5 * <l^3>/<l^2>, and note that the percolation parameter equals to P32*<l^3>/<l^2>
				Excluded_volume_4 = Excluded_volume_3 / (2 * C2 * ((pow(max_R, 3) - pow(min_R, 3)) / 3));

				//********below is 0.5 * <A*P>/<l^2>, and note that the percolation parameter equals to P32*<l^3>/<l^2>
				Excluded_volume_5 = Excluded_volume_1 / (2 * C2 * ((pow(max_R, 3) - pow(min_R, 3)) / 3));
			}
			else if (str_frac_size == "single")
			{
				double R = min_R;
				//********below is 0.5*<A*P>
				Excluded_volume_1 = 0.5 * (2 * R * R) * (4 * pow(2, 0.5) * R);

				//********below is 0.5*<A>*<P>
				Excluded_volume_2 = Excluded_volume_1;

				//********below is 0.5 * <l^3>
				Excluded_volume_3 = 0.5 * 2 * pow(2, 0.5) * R * R * R;

				//********below is 0.5 * <l^3>/<l^2>, and note that the percolation parameter equals to P32*<l^3>/<l^2>
				Excluded_volume_4 = Excluded_volume_3 / (2 * R * R);

				//********below is 0.5 * <A*P>/<l^2>, and note that the percolation parameter equals to P32*<l^3>/<l^2>
				Excluded_volume_5 = Excluded_volume_1 / (2 * R * R);
			}
		}
		else if (str_ori == "fisher")
		{
			...
			if (str_frac_size == "powerlaw")
			{
				...
				//********below is 2/pi * < sinγ > *<A>*<P>
				Excluded_volume_2 = (8 * pow(2, 0.5) * C2 * C2 * (pow(x1, 3 - alpha_g) / (3 - alpha_g) - pow(x0, 3 - alpha_g) / (3 - alpha_g)) * (pow(x1, 2 - alpha_g) / (2 - alpha_g) - pow(x0, 2 - alpha_g) / (2 - alpha_g)));
				Excluded_volume_2 = 2 / pi * SinGamma * Excluded_volume_2;

				//********below is 2/pi * < sinγ > * <l^3>
				Excluded_volume_3 = 2 * pow(2, 0.5) * C2 * pow(x1, 4 - alpha_g) / (4 - alpha_g) - 2 * pow(2, 0.5) * C2 * pow(x0, 4 - alpha_g) / (4 - alpha_g);
				Excluded_volume_3 = 2 / pi * SinGamma * Excluded_volume_3;
				...
				//std::cout << "kappa is: " << kappa << "\n";
				//std::cout << "sinGamma is: " << SinGamma << "\n";*/
			}
			else if (str_frac_size == "lognormal")
			{
				mean = log(mean);
				var = log(var);
				//********below is 2/pi * < sinγ > * < A * P >
				Excluded_volume_1 = 2 / pi * SinGamma * 8 * pow(2, 0.5) * exp(3 * mean + (9 / 2) * (var * var));

				//********below is 2/pi * < sinγ > *<A>*<P>
				Excluded_volume_2 = 2 / pi * SinGamma * 2 * exp(2 * mean + 2 * var * var) * 4 * pow(2, 0.5) * exp(mean + 0.5 * var * var);

				//********below is 2/pi * < sinγ > * <l^3>
				Excluded_volume_3 = 2 / pi * SinGamma * 2 * pow(2, 0.5) * exp(3 * mean + (9 / 2) * (var * var));
			...
			else if (str_frac_size == "uniform")
			{
				double C2 = 1 / (max_R - min_R);
				//********below is 2/pi * < sinγ > * < A * P >
				Excluded_volume_1 = 2 / pi * SinGamma * 2 * pow(2, 0.5) * C2 * (pow(max_R, 4) - pow(min_R, 4));

				//********below is 2/pi * < sinγ > *<A>*<P>
				Excluded_volume_2 = 2 / pi * SinGamma * C2 * C2 * (8 * pow(2, 0.5) / (2 * 3)) * (pow(max_R, 3) - pow(min_R, 3)) * (pow(max_R, 2) - pow(min_R, 2));

				//********below is 2/pi * < sinγ > * <l^3>
				Excluded_volume_3 = 2 / pi * SinGamma * 2 * pow(2, 0.5) * C2 * ((pow(max_R, 4) - pow(min_R, 4)) / 4);
			...
			else if (str_frac_size == "single")
			{
				double R = min_R;
				//********below is 2/pi * < sinγ > * < A * P >
				Excluded_volume_1 = 2 / pi * SinGamma * (2 * R * R) * (4 * pow(2, 0.5) * R);

				//********below is 2/pi * < sinγ > *<A>*<P>
				Excluded_volume_2 = Excluded_volume_1;

				//********below is 2/pi * < sinγ > * <l^3>
				Excluded_volume_3 = 2 / pi * SinGamma * 2 * pow(2, 0.5) * R * R * R;
	...
	inline void Domain::Correlation_length_and_gyration_radius()
	{
		...
		if (Listofclusters.Size() != 0)
		{
			...
			if (Listofclusters[max_cluster].Size() > 1)
			{
				...
				for (size_t i = 0; i < Listofclusters[max_cluster].Size(); ++i)
				{
					Vec3_t distance = Fractures[Listofclusters[max_cluster][i]].Center - Center_of_cluster;
				...
				for (size_t i = 0; i < Listofclusters[max_cluster].Size(); ++i)
				{
					for (size_t j = 0; j < Listofclusters[max_cluster].Size(); ++j)
					{
						if (Listofclusters[max_cluster][j] > Listofclusters[max_cluster][i])
						{
							Vec3_t distance = Fractures[Listofclusters[max_cluster][i]].Center - Fractures[Listofclusters[max_cluster][j]].Center;
							double module = pow(pow(distance[0], 2) + pow(distance[1], 2) + pow(distance[2], 2), 0.5);
							total_distance = total_distance + module;
						}
					}
				}
				Xi = total_distance / ((Listofclusters[max_cluster].Size() - 1) * Listofclusters[max_cluster].Size() * 0.5);
				double Model_volume = (Model_domain(0) - Model_domain(1)) * (Model_domain(3) - Model_domain(2)) * (Model_domain(5) - Model_domain(4));
