...
//	        set_to_zero(Particles[i]->FSISigma);
        ...
        set_to_zero(Particles[i]->SumStress);
		Particles[i]->S		= 0.0;
	}
	...
	if ((rij/h)<=Cellfac)
	{
		...
		if (DeltaSPH>1.0e-12)
		{
			...
			if (!P2->IsFree) Cj = SoundSpeed(P1->PresEq, P1->Cs, dj, P1->RefDensity); else Cj = SoundSpeed(P2->PresEq, P2->Cs, dj, P2->RefDensity);
               ...
               //delfil = P1->DeltaSPH*(0.5*(Ci+Cj))*(dj-di)*(dot( xij, GK*xij )/(rij + 0.1*h)); // delta-SPH from Ferrari et al. (2009).
...
//				if (P1->IsFree) vab(0) = P1->v(0) + P2->vb(0); else vab(0) = -P1->vb(0) - P2->v(0);
		...
		Mat3_t StrainRateP1,StrainRateP2,RotationRateP1,RotationRateP2;
		set_to_zero(StrainRateP1);
        set_to_zero(StrainRateP2);
		set_to_zero(RotationRateP1);
        set_to_zero(RotationRateP2);
        ...
        if (P1->ID==-100 && P2->ID==-100) std::cout << " A " << std::endl;
        if (!KGcorrection){
            tcorrGKP1 = GK*xij;
            tcorrGKP2 = tcorrGKP1;
        }
        else if (KGcorrection){
            ...
            /*if (P1->ID== -100){
            std::cout << "MatrixO: " << GK*xij << std::endl;
            std::cout << "Matrix1: " << tcorrGKP1 << std::endl;
            std::cout << "Matrix2: " << tcorrGKP2 << std::endl;
            }*/
        }

        if (P1->ID==-100 && P2->ID==-100) std::cout << " B " << std::endl;

		// Locking the particle 1 for updating the properties
		omp_set_lock(&P1->my_lock);

        // Calculation strain rate tensor
        StrainRateP1(0,0) = 2.0*vab(0)*tcorrGKP1(0);
        StrainRateP1(0,1) = vab(0)*tcorrGKP1(1)+vab(1)*tcorrGKP1(0);
        StrainRateP1(0,2) = vab(0)*tcorrGKP1(2)+vab(2)*tcorrGKP1(0);
        StrainRateP1(1,0) = StrainRateP1(0,1);
        StrainRateP1(1,1) = 2.0*vab(1)*tcorrGKP1(1);
        StrainRateP1(1,2) = vab(1)*tcorrGKP1(2)+vab(2)*tcorrGKP1(1);
        StrainRateP1(2,0) = StrainRateP1(0,2);
        StrainRateP1(2,1) = StrainRateP1(1,2);
        StrainRateP1(2,2) = 2.0*vab(2)*tcorrGKP1(2);
        StrainRateP1      = -0.5 * StrainRateP1;

        // Calculation rotation rate tensor
        RotationRateP1(0,1) = vab(0)*tcorrGKP1(1)-vab(1)*tcorrGKP1(0);
        RotationRateP1(0,2) = vab(0)*tcorrGKP1(2)-vab(2)*tcorrGKP1(0);
        RotationRateP1(1,2) = vab(1)*tcorrGKP1(2)-vab(2)*tcorrGKP1(1);
        RotationRateP1(1,0) = -RotationRateP1(0,1);
        RotationRateP1(2,0) = -RotationRateP1(0,2);
        RotationRateP1(2,1) = -RotationRateP1(1,2);
        RotationRateP1      = -0.5 * RotationRateP1;


        // Calculating the forces for the particle 1 & 2
        Vec3_t tempP1S = 0.0;
        if (GradientType == 0)
            Mult( tcorrGKP1 , ( 1.0/(di*di)*Sigmai + 1.0/(dj*dj)*Sigmaj + PIij + TIij ) , tempP1S);
        else
            Mult( tcorrGKP1 , ( 1.0/(di*dj)*(Sigmai + Sigmaj)           + PIij + TIij ) , tempP1S);
        if (Dimension == 2) tempP1S(2) = 0.0;

        double tempP1D = 0.0;
        tempP1D = dot( vij , tcorrGKP1 );

		P1->a		+= mj * tempP1S;
		P1->dDensity	+= mj * (di/dj) * tempP1D;

		if (P1->IsFree)
		{
			P1->ZWab	+= mj/dj* K;
			P1->StrainRate	 = P1->StrainRate + mj/dj*StrainRateP1;
			P1->RotationRate = P1->RotationRate + mj/dj*RotationRateP1;
			if (SWIType ==1) P1->S = P1->S + mj/dj*vab(0)*xij(1)*-GK;
		}
		else
			P1->ZWab	= 1.0;
		...
		if (P1->Shepard)
			if (P1->ShepardCounter == P1->ShepardStep){
				P1->SumDen += mj*    K;
                P1->SumStress = P1->SumStress + (mj/dj)*K*Sigmaj;
            }
		omp_unset_lock(&P1->my_lock);
        ...
        // Calculation strain rate tensor
        StrainRateP2(0,0) = 2.0*vab(0)*tcorrGKP2(0);
        StrainRateP2(0,1) = vab(0)*tcorrGKP2(1)+vab(1)*tcorrGKP2(0);
        StrainRateP2(0,2) = vab(0)*tcorrGKP2(2)+vab(2)*tcorrGKP2(0);
        StrainRateP2(1,0) = StrainRateP2(0,1);
        StrainRateP2(1,1) = 2.0*vab(1)*tcorrGKP2(1);
        StrainRateP2(1,2) = vab(1)*tcorrGKP2(2)+vab(2)*tcorrGKP2(1);
        StrainRateP2(2,0) = StrainRateP2(0,2);
        StrainRateP2(2,1) = StrainRateP2(1,2);
        StrainRateP2(2,2) = 2.0*vab(2)*tcorrGKP2(2);
        StrainRateP2      = -0.5 * StrainRateP2;

        // Calculation rotation rate tensor
        RotationRateP2(0,1) = vab(0)*tcorrGKP2(1)-vab(1)*tcorrGKP2(0);
        RotationRateP2(0,2) = vab(0)*tcorrGKP2(2)-vab(2)*tcorrGKP2(0);
        RotationRateP2(1,2) = vab(1)*tcorrGKP2(2)-vab(2)*tcorrGKP2(1);
        RotationRateP2(1,0) = -RotationRateP2(0,1);
        RotationRateP2(2,0) = -RotationRateP2(0,2);
        RotationRateP2(2,1) = -RotationRateP2(1,2);
        RotationRateP2      = -0.5 * RotationRateP2;


        // Calculating the forces for the particle 1 & 2
        Vec3_t tempP2S = 0.0;
        if (GradientType == 0)
            Mult( tcorrGKP2 , ( 1.0/(di*di)*Sigmai + 1.0/(dj*dj)*Sigmaj + PIij + TIij ) , tempP2S);
        else
            Mult( tcorrGKP2 , ( 1.0/(di*dj)*(Sigmai + Sigmaj)           + PIij + TIij ) , tempP2S);
        if (Dimension == 2) tempP2S(2) = 0.0;

        double tempP2D = 0.0;
        tempP2D = dot( vij , tcorrGKP2 );

		P2->a		-= mi * tempP2S;
		P2->dDensity	+= mi * (dj/di) * tempP2D;
		if (P2->IsFree)
		{
			P2->ZWab	+= mi/di* K;
			P2->StrainRate	 = P2->StrainRate + mi/di*StrainRateP2;
			P2->RotationRate = P2->RotationRate + mi/di*RotationRateP2;
			if (SWIType ==1) P2->S = P2->S + mi/di*vab(0)*xij(1)*-GK;
		}
		else
			P2->ZWab	= 1.0;
		...
		if (P2->Shepard){
			if (P2->ShepardCounter == P2->ShepardStep)
				P2->SumDen += mi*    K;
                P2->SumStress = P2->SumStress + (mi/di)*K*Sigmai;
        }
